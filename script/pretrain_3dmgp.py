...
def train(rank, config, world_size, verbose=1):
    ...
    elif config.model.backbone == 'SchNet':
        rep = layers.SchNet(in_node_nf=config.model.max_atom_type * (config.model.charge_power + 1), out_node_nf = config.model.hidden_dim,
                                        num_filters = config.model.hidden_dim, in_edge_nf=edge_types, hidden_nf=config.model.hidden_dim, num_interactions=config.model.n_layers)
    elif config.model.backbone == 'ET':
        rep = layers.TorchMD_ET(hidden_channels = config.model.hidden_dim, num_layers = config.model.n_layers, max_z = config.model.max_atom_type * (config.model.charge_power + 1),
                            num_edges = edge_types)

    model = models.EquivariantDenoisePred(config, rep)
        ...
        for batch in dataloader:
            ...
            if verbose and (batch_cnt % config.train.log_interval == 0 or (epoch==0 and batch_cnt <= 10)):
                print('Epoch: %d | Step: %d | loss: %.5f(%.3f/%.3f)| GradNorm: %.5f | Lr: %.5f' % \
                                    (epoch + start_epoch, batch_cnt, batch_losses[-1], loss_pred_noise.item(), loss_denoise.item(), norm.item(), optimizer.param_groups[0]['lr']))
