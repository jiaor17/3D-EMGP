...
from data.dataset import BatchDatapoint, GEOMDataset3DInfomax, AtomOnehot, EdgeHop, Cutoff
import json
...
def train(rank, config, world_size, verbose=1):
    ...
    val_dataset = GEOMDataset3DInfomax([val_block],val_block_size, transforms=transform)
    ...
    train_dataset = GEOMDataset3DInfomax(train_blocks, train_block_size, transforms=transform)
    ...
    model = baselines.Infomax3D(config, rep)
