...
from data.dataset import BatchDatapoint, GEOMDatasetCL, AtomOnehot, EdgeHop, Cutoff
import json
...
def train(rank, config, world_size, verbose=1):
    ...
    val_dataset = GEOMDatasetCL([val_block],val_block_size, transforms=transform)
    ...
    train_dataset = GEOMDatasetCL(train_blocks, train_block_size, transforms=transform)
    ...
    model = baselines.GraphCL(config, rep)
...
def main():
    ...
    if config.train.save and config.train.save_path is not None:
        config.train.save_path = os.path.join(config.train.save_path, config.model.name, 'GraphCL')
