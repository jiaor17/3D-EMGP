...
from data.dataset import BatchDatapoint, GEOMDatasetGPT, AtomOnehot, EdgeHop, Cutoff
import json
...
def train(rank, config, world_size, verbose=1):
    ...
    val_dataset = GEOMDatasetGPT([val_block],val_block_size, transforms=transform)
    ...
    train_dataset = GEOMDatasetGPT(train_blocks, train_block_size, transforms=transform)
    ...
    model = baselines.GPT(config, rep)
...
def main():
    ...
    if config.train.save and config.train.save_path is not None:
        config.train.save_path = os.path.join(config.train.save_path, config.model.name, 'GPT')
