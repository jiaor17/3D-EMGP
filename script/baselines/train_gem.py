...
from data.dataset import BatchDatapoint, GEOMDatasetGEM, AtomOnehot, EdgeHop, Cutoff
import json
...
def train(rank, config, world_size, verbose=1):
    ...
    val_dataset = GEOMDatasetGEM([val_block],val_block_size, transforms=transform)
    ...
    train_dataset = GEOMDatasetGEM(train_blocks, train_block_size, transforms=transform)
    ...
    # rep = layers.TorchMD_ET(hidden_channels = config.model.hidden_dim, num_layers = config.model.n_layers, max_z = config.model.max_atom_type)

    model = baselines.GemPretrain(config, rep)
...
def main():
    ...
    if config.train.save and config.train.save_path is not None:
        config.train.save_path = os.path.join(config.train.save_path, config.model.name, 'GEM')
