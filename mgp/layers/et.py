...
class TorchMD_ET(nn.Module):
    ...
    def __init__(
        ...
        self.neighbor_embedding = (
            NeighborEmbedding(
                hidden_channels, num_rbf + num_edges, cutoff_lower, cutoff_upper, self.max_z
            ).jittable()
    ...
    def forward(self, z, pos, edge_index=None, edge_type=None, batch=None):
        x = self.embedding(z)

        edge_index, edge_weight, edge_vec = self.distance(pos, batch, edge_index = edge_index)
        assert (
...
class TorchMD_Net(nn.Module):
    def __init__(
        ...
        if self.property == 'mu':
            self.output_model = EquivariantDipoleMoment(self.hidden_dim, self.act) if self.equivariant else DipoleMoment(self.hidden_dim, self.act)
        elif self.property == 'r2':
            self.output_model = ElectronicSpatialExtent(self.hidden_dim, self.act)
        else:
            self.output_model = EquivariantScalar(self.hidden_dim, self.act) if self.equivariant else Scalar(self.hidden_dim, self.act)
        ...
        self.reset_parameters(pretrained = pretrained)

    def reset_parameters(self, pretrained):
        if not pretrained and hasattr(self.representation_model,'reset_parameters'):
            self.representation_model.reset_parameters()
        self.output_model.reset_parameters()
        if self.prior_model is not None:
            self.prior_model.reset_parameters()

    def forward(self, x, z, pos, edge_index, edge_attr, batch, atom_mask = None):
        ...
        x, v = self.representation_model(x, pos, edge_index, edge_attr)
